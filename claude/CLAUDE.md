...
ターミナルで `claude --continue`（このセッションの作業ディレクトリで実行）すれば同じ会話のまま無人続行でき、
`claude-auto-retry`（tmux 監視で制限解除後に自動継続。導入時は npm 経由・`~/.zshrc` にマーカー区切りで
シェル関数を追加）を導入済みなら制限を跨いでも止まらない。提案は一言で済ませ、ブロッキングな確認にはしない
（デスクトップ続行を選べばそのまま進める）。短時間で終わるタスクには提案しない。詳細経緯は dotfiles メモリ
`claude-code-overnight-autoresume.md` 参照。
