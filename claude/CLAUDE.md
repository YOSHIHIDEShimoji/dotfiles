...
## 長時間・無人タスクの CLI 提案（claude-auto-retry）

デスクトップアプリは 5 時間の使用量制限に達すると停止したまま自動再開しない。「寝るからやっといて」「放置するから」
「朝までに」等、無人で数時間規模になる兆候を検知したら、作業に入る前に一言、CLI への切り替えを提案する。

`claude --resume <id>` は ID を指定してもカレントディレクトリ基準で検索する（実測確認済み：別ディレクトリから
叩くと `No conversation found` で失敗する）。ID を指定しても `cd` は省略できない。提案時は必ず**実際の絶対パス**
と**実際のセッションID**に置き換えて出す（プレースホルダのまま出さない。ID は `$CLAUDE_CODE_SESSION_ID` 環境変数
で取得できる）。`--continue`（最新セッションを自動選択）ではなく `--resume <id>` を使うのは、同じディレクトリに
複数セッションがある場合の「最新のつもりが別セッションだった」という取り違えを避けるため：

```bash
cd /実際の/絶対パス
claude --resume 実際のセッションID
```

`claude-auto-retry`（tmux 監視で制限解除後に自動継続。導入済み・`CLAUDE_AUTO_RETRY_LAUNCH_WRAPPER` で
`caffeinate` によるスリープ防止も統合済み・`install-timer` で 5 分おきの自己修復も導入済み）が入っていれば
制限を跨いでも止まらない。呼び出しごとに一意な tmux セッション名（`claude-retry-<PID>-<timestamp>`）が
使われるため、`cd` と ID さえ正しければ複数プロジェクトを真に並行実行できる。

**注意（実測で確認した既知の落とし穴）:** tmux を経由せず素の `claude` を直接叩いた場合、そのプロセスは
`claude-auto-retry reconcile` の監視対象に一切乗らない（tmux pane が無いと capture-pane/send-keys の
仕組み自体が使えないため、後から監視を追加することも不可能）。無人運用させたいセッションは必ずラッパー経由
（＝新しいターミナル/tmux ウィンドウで `claude` コマンドを叩く。既存の素のプロセスを見つけた場合は、
それを終了してから同じ `cd` + `claude --resume <id>` で改めて起動し直すよう案内する）で開始することを徹底する。

提案は一言で済ませ、ブロッキングな確認にはしない（デスクトップ続行を選べばそのまま進める）。
短時間で終わるタスクには提案しない。詳細経緯は dotfiles メモリ `claude-code-overnight-autoresume.md` 参照。
